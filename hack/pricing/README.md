...
- `internal/pricing/flat.json`: flat rates per region: Elastic IP, NAT gateway, ALB, NLB and CLB USD/hour, and EBS snapshot USD per GB-month, from the EC2, AWSELB and AmazonVPC offer files. Refreshed for the same ten regions; the other six were seeded from the same offer files and are kept. The CloudWatch log storage and RDS storage and backup rates (`log_storage_gb_month`, `rds_*_gb_month`) are hand-seeded and kept, not fetched.
